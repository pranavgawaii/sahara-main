            ...
            <div className="max-w-[1240px] mx-auto px-6 md:px-20 relative z-10">
                ...
                >
                    {testimonials.map((testimonial, index) => (
                        ...
                        >
                            <Card className="h-full border-none shadow-md hover:shadow-2xl transition-all duration-300 bg-white dark:bg-[#111] hover:-translate-y-2 group">
                                <CardContent className="p-8 flex flex-col h-full">
                                    <Quote className="w-10 h-10 text-slate-300 dark:text-white/10 mb-6 fill-current group-hover:text-blue-500/20 transition-colors" />
                                    <p className="text-slate-700 dark:text-slate-300 italic font-inter mb-6 leading-relaxed flex-grow">
                                        "{testimonial.content}"
                                    </p>
                                    <div className="flex items-center gap-4 mt-auto">
                                        <Avatar className="h-12 w-12 border-2 border-slate-200 dark:border-white/10 shadow-sm">
                                            <AvatarFallback className="bg-gradient-to-br from-blue-500 to-purple-500 text-white font-bold">
                                                {testimonial.avatar}
                                            </AvatarFallback>
                                        </Avatar>
                                        <div>
                                            <h4 className="font-bold text-slate-900 dark:text-white font-dm">{testimonial.author}</h4>
                                            <p className="text-sm text-slate-500 font-inter">{testimonial.role}</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
